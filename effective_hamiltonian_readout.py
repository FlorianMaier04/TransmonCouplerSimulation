...
class Setup:    
    ...
    def compute_U(self, fre, amp, show_progress=True):
        ...
        for col_idx, initial_state_idx in iterator:
            ...
            # Evolve
            result = mesolve(
                [self.H0_dressed, [self.V1_dressed, lambda t, args: amp * np.cos(fre * t)]],
                initial_state,
                tlist,
                []
            )
...
def configure_simulation():
    config = {
        ...
        'parameter_preset': 'parameters_fast',
        # Options: 'parameters_fast', 'parameters_slow', 'parameters_high_range', 'parameters_side'
...
def run_simulation(simulator, config):
    ...
    print(f"\n{'='*70}")
    print(f"Running simulation for element: ({i},{j})")
    ...
    element_label = f'$\delta_{{{i}}}$-$\delta_{{{j}}}$' if extract_func == 'diag_diff' else f'Ω_{{{i},{j}}}'
    ...
    info_text = (
        ...
        f"T={simulator.time} ns | Δt={simulator.tstep} ns | Element: ({i},{j})"
    )
...
if __name__ == "__main__":
    ...
    fig = run_simulation(simulator, config)
    plt.show()
