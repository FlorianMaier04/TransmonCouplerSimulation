...
class Simulator:
    ...
    def config(self, config):
        self.tg = config['tgate']
        self.order = config['order']
        self.setup_modulation(config['pulse_shape'], config['pulse_args'])
        self.base_amplitude = config['base_amplitude']*2*np.pi
        self.base_wd = (self.res_freq_static if(config['base_wd']=='res') else config['base_wd']) * 2*np.pi
        self.base_wd += config['wd_detune']*2*np.pi