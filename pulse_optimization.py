...
# ------------------------------------------------------------------
# 1. Symbolische Vorberechnung (Einmalige Ausführung)
# ------------------------------------------------------------------
def prepare_floquet_functions(
  ...
  try:
    ...
    Heff = Heff_Floquet_total_matrix_summed(
        ...
        V0=None,
        ref_state=None,
        dwd=wd_expr.diff(t_sym),
        dA=A_expr.diff(t_sym),
        t=t_sym,
        analytics=True,
        rW=rW,
    ...
    for l in range(D):
      for idx_a, a in enumerate(resonances):
        for p, (W_lp_a, _, _) in W[r, l, a].items():
          M_sym[l, idx_a] += (
              sy.exp(-sy.I * (s.E_array[kref] + p * wd_expr) * t_sym) * W_lp_a
          )
  ...
  return (
      sy.lambdify(
          args_list, sy.Matrix(Heff).doit(), modules=["numpy", "scipy"]
      ),
...
def _eval_H_stack(H_func, t_mids, p_vals, d_res):
  ...
  for k, t_mid in enumerate(t_mids):
    H_all[:, :, k] = np.array(
        H_func(t_mid, *p_vals), dtype=complex
    ).reshape(d_res, d_res)
  return H_all
...
def compute_pulse_infidelity(
  ...
  M_0 = np.array(M_func(0, *p_vals), dtype=complex).reshape(D, d_res)
  U_total = M_final @ U_eff @ M_0
  norms = np.linalg.norm(U_total, axis=0)
  U_total /= norms
  if target_metric == "population":
    fidelity = 0.5 * (
        np.abs(U_total[state_b, ind_a]) ** 2
        + np.abs(U_total[state_a, ind_b]) ** 2
    )

  elif target_metric == "process":
    U_target_mat = (
        qobj_ideal.full() if hasattr(qobj_ideal, "full") else qobj_ideal
    )
    trace_val = np.trace(U_target_mat.conj().T @ U_eff)
...
def optimize_pulse_parameters(
  ...
  def callback(xk, convergence):
    ...
    if verbose:
      ...
      print(
          f"[DEBUG Gen Callback] Aufrufe: {n_calls:6d} | Bisher beste"
          f" Infidelity: {best_so_far:.6e} | Conv: {convergence:.4e} | Zeit:"
          f" {elapsed:.2f}s"
      )
  ...
  result = differential_evolution(
      ...
      maxiter=maxiter,
      tol=tol,
      polish=False,  # Polish wird unten manuell kontrolliert
      updating="deferred",
      workers=workers,
      disp=verbose,
      seed=seed,
...
# ------------------------------------------------------------------
# 4. Gate Time Refinement (Phase Alignment via T = 2pi / wd)
# ------------------------------------------------------------------
def refine_gate_time(
    best_p_pop,
    tg_target,
    wd_val,
    H_func,
  ...
  [tg_target - T/2, tg_target + T/2] begrenzt, um die Rotationsphase der
  Process-Fidelity anzupassen.
  """
  ...
  D, d_res = len(s.E_array), len(resonances)
  ...
  def obj_tg(tg_cand):
    ...
    t_mids = np.linspace(0, tg_cand, N_steps)[:-1] + dt / 2.0
  ...
  bounds = (tg_target - 0.5 * T_period, tg_target + 0.5 * T_period)
