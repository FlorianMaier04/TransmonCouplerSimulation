...
def _compute_floquet_step(it, t, dt, rH, rW, signal, resonances,
    ...
    if not num:
        ...
        for s in range(d_res):
            phase = np.exp(1j * integral_gamma[s] - 1j * integral_theta[s])
                        ...
                        for r2 in range(rW - r1 + 1):
                            for s in range(d_res):
                                for p1, W1 in W[r1, l, a].items():
                                    for p2, W2 in W0[r2, k, b].items():
                                        if p1 not in integral_beta:
                                            integral_beta[p1] = 0.0
                                        phase = np.exp(
                                            -1j * E[kref] * t - 1j * integral_beta[p1]
                                        )
                                        evol[l] += (
                                            phase
                                            * W1
            ...
            for a in resonances:
                for p in W[r, l, a]:
                    if p not in integral_beta:
                        integral_beta[p] = 0.0
                    integral_beta[p] += p * wd * dt

    evol /= np.linalg.norm(evol)
    return evol, U, uprev, u0, integral_beta, integral_theta, integral_gamma
...
def Psi_t_FloquetPerturb_dynamic(rH, rW, signal, resonances,
    ...
    dt = tlist[-1]/len(tlist)
    uprev = None
    u0 = None
    wd0, amp0 = signal(0)
    W0 = W_Floquet_elements(rW,wd0,resonances,E,amp0/2 * V_posHarm,analytics=False)
...
def find_optimal_time_near_tg(rH, rW, tg, signal, a, b, c, E0, V1,
                              window=2, n_points=200):
    """
    Find the time closest to tg that maximizes fidelity with an iSWAP gate
    using only the Floquet time-step logic around the requested interval.

    The routine evaluates the evolution only on the time grid near tg and uses
    the internal time-step helper ``_compute_floquet_step`` directly. This keeps
    the beta-phase accumulation and the special ``u0``/``W0`` initialization
    consistent while avoiding the extra work of evolving a full state history.
    """
    t_min = max(0.0, tg - window)
    t_max = tg
    tlist = np.linspace(t_min, t_max, n_points)

    resonances = {a: 1, b: 0, c: 2}
    d_res = len(resonances)
    kref = min(resonances.keys())
    ind = {state: idx for idx, state in enumerate(resonances)}

    initial_state_a = np.zeros(len(E0), dtype=complex)
    initial_state_a[a] = 1
    initial_state_b = np.zeros(len(E0), dtype=complex)
    initial_state_b[b] = 1
    initial_state_c = np.zeros(len(E0), dtype=complex)
    initial_state_c[c] = 1
    initial_state_list = [initial_state_a, initial_state_b, initial_state_c]
    ...
    for j, initial_state in enumerate(initial_state_list):
        ...
        wd0, amp0 = signal(tlist[0])
        W0 = W_Floquet_elements(
            rW, wd0, resonances, E0, amp0 / 2 * V1, analytics=False
        )
        ...
        for it, t in enumerate(tlist):
            if it == 0:
                ...
                dt = tlist[it] - tlist[it - 1]

            evol, U, uprev, u0, integral_beta, integral_theta, integral_gamma = _compute_floquet_step(
                it, t, dt, rH, rW, signal, resonances, E0, V1,
                initial_state, len(E0), d_res, ind, kref, integral_beta,
                integral_theta, integral_gamma, uprev, u0, U, W0, num=True,
            )
    ...
    fidelities = np.zeros(len(tlist), dtype=float)
    for k in range(len(tlist)):
        trace_overlap = np.trace(U_ideal.conj().T @ lU[k])
        fidelities[k] = np.abs(trace_overlap) ** 2 / 3**2

    idx_opt = int(np.argmax(fidelities))
    return tlist[idx_opt], fidelities[idx_opt], lU[idx_opt], fidelities, tlist
